#!/bin/sh
#-----------------------------------------------------------------------------
# SpaceDub - generate the Xcode project
#
...
CMAKE_ARGS="$CMAKE_ARGS -DSPACEDUB_BUILD_AU=$BUILD_AU"
...
echo
if [ "$GENERATOR" = "Xcode" ]; then
	echo "Xcode project written to $BUILD_DIR/SpaceDub.xcodeproj"
	echo "Or build from the command line:"
	echo "    cmake --build $BUILD_DIR --config Release"
	if [ "$BUILD_AU" = "ON" ]; then
		echo
		echo "Building the SpaceDub-au scheme also produces SpaceDub.component and"
		echo "copies it to ~/Library/Audio/Plug-Ins/Components/."
	fi
	if [ "$OPEN_XCODE" = "1" ] && [ "$(uname)" = "Darwin" ]; then
		open "$BUILD_DIR/SpaceDub.xcodeproj"
	fi
else
