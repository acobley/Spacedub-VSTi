#!/bin/bash
#-----------------------------------------------------------------------------
# Build a distributable .pkg installer for SpaceDub.
#
...
#
# The result is installer/SpaceDub-<version>.pkg, which installs
#
...
NAME="SpaceDub"
VST3_ID="audio.spacedub.vst3"
AU_ID="audio.spacedub.audiounit"
...
echo "==> SpaceDub $VERSION, from $CONFIG"
...
# THE POSTINSTALL SCRIPT MUST BE EXECUTABLE, and this is not something to
# assume. pkgbuild takes --scripts exactly as it finds the directory, and a
...
# out. cp, zip, AirDrop, a file share and an editor that rewrites in place have
# all been seen to drop the bit. So stage the directory and set it here, where
# it cannot have been lost in transit.
rm -rf "$WORK/scripts-au"
mkdir -p "$WORK/scripts-au"
cp "$HERE/scripts-au/postinstall" "$WORK/scripts-au/postinstall"
chmod +x "$WORK/scripts-au/postinstall"
[ -x "$WORK/scripts-au/postinstall" ] || {
    echo "build-installer: postinstall is not executable and chmod did not fix it" >&2
    exit 1
}
