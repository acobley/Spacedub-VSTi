#!/bin/bash
#-----------------------------------------------------------------------------
# Check that an INSTALLED SpaceDub actually works on this machine.
#
...
echo "SpaceDub install check - $(sw_vers -productName 2>/dev/null) $(sw_vers -productVersion 2>/dev/null) on $(uname -m)"
echo
...
#-----------------------------------------------------------------------------
receipts="$(pkgutil --pkgs 2>/dev/null | grep -i spacedub || true)"
if [ -n "$receipts" ]; then
    while IFS= read -r r; do pass "$r"; done <<< "$receipts"
else
    note "no SpaceDub receipts - installed by hand rather than by the .pkg?"
fi
...
if command -v auval >/dev/null 2>&1; then
    ...
    if auval -v aufx SDub AECo 2>&1 | grep -q "AU VALIDATION SUCCEEDED"; then
        pass "auval -v aufx SDub AECo - AU VALIDATION SUCCEEDED"
    else
        fail "auval did not report success"
        note "Run it yourself for the detail:  auval -v aufx SDub AECo"
...
if [ "$FAILED" -eq 0 ]; then
    ...
    echo "Remember the Enabled switch starts OFF - until it is lit SpaceDub"
    echo "passes audio straight through, which reads exactly like a dead plug-in."
    exit 0
fi
